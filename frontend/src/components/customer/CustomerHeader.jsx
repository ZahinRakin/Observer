...
const CustomerHeader = ({ user, onViewProfile, onEditProfile }) => {
      ...
      <div className="flex items-center gap-6">
        <div className="flex flex-col">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-bold bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent tracking-tight">Customer Dashboard</h1>
          </div>
          <span className="text-gray-300 text-sm ml-11 mt-1">Welcome back, {user ? user.username : 'Customer'}</span>
        </div>
        ...
        {/* <div className="hidden lg:flex items-center gap-6 ml-8">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-300">Platform Online</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 bg-gray-700 rounded-full flex items-center justify-center">
              <div className="w-3 h-3 bg-blue-400 rounded-full"></div>
            </div>
            <span className="text-sm text-gray-300">3 Subscribed Products</span>
          </div>
        </div> */}
      </div>
      ...
      <div className="flex items-center gap-4">
        ...
        >
            ...
            <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-700 transition-all duration-200">
              <div className="relative">
                <img
                  ...
                  alt="Avatar"
                  className="w-10 h-10 rounded-full object-cover border-2 border-gray-600 group-hover:border-blue-500 transition-all duration-200 shadow-sm"
                />
                {/* <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-green-400 rounded-full border-2 border-gray-800"></div> */}
              </div>
              <div className="hidden lg:flex flex-col items-start">
                <span className="text-sm font-medium text-white">{user?.username || 'Customer'}</span>
                {/* <span className="text-xs text-gray-400">Premium Customer</span> */}
              </div>
          ...
          {showMenu && (
            ...
            >
              {/* Profile Section */}
              <div className="px-4 py-3 border-b border-gray-600 bg-gradient-to-r from-blue-900/50 to-purple-900/50">
                <div className="flex items-center gap-3">
                  <img
                    src={user?.avatar || "/avatar.jpg"}
                    alt="Avatar"
                  ...
                  />
                  <div>
                    <div className="font-medium text-white text-sm">{user?.username || 'Customer'}</div>
                    <div className="text-xs text-gray-400">{user?.email || 'customer@example.com'}</div>
                  </div>
                </div>
              </div>
